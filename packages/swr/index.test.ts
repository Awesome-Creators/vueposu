  ...
  it("should broadcast isValidating", async () => {
    ...
    const Initiator = defineComponent({
      ...
      setup() {
        ...
        onUnmounted(() => clearTimeout(timer as NodeJS.Timeout));
        return {
          isValidating,
          revalidate,
        };
...
describe("useSWR - refresh", () => {
  ...
  it("should update data upon interval changes", async () => {
    ...
    const component = mount(
      defineComponent({
        template: `count: {{ data }} <button @click="click"></button>`,
        setup() {
          ...
          const click = () =>
            (refreshInterval.value =
              refreshInterval.value < 400 ? refreshInterval.value + 100 : 0);

          return { data, refreshInterval, click };
        },
      })
    );
    ...
    const component = mount(
      defineComponent({
        ...
        setup() {
          ...
          const click = () => (flag.value = 0);

          return { data, flag, click };
        },
      })
    );
...
describe("useSWR - error", () => {
  ...
  it("should not trigger the onLoadingSlow and onSuccess event after component unmount", async () => {
    ...
    const component = mount(
      defineComponent({
        components: { Block },
        template: `<Block v-if="on" /> <button @click="click"></button>`,
        setup() {
          const on = ref(true);
          const click = () => (on.value = !on.value);
...
describe("useSWR - focus", () => {
  ...
  it("revalidateOnFocus shoule be stateful", async () => {
    ...
    const component = mount(
      defineComponent({
        template: `data: {{ data }} <button @click="click"></button>`,
        setup() {
          ...
          const click = () =>
            (revalidateOnFocus.value = !revalidateOnFocus.value);

          return { data, revalidateOnFocus, click };
        },
      })
    );
    ...
    const component = mount(
      defineComponent({
        ...
        setup() {
          ...
          const click = () => boundMutate("mutated", false);

          return { data, boundMutate, click };
        },
      })
    );
...
describe("useSWR - key", () => {
    ...
    const component = mount(
      defineComponent({
        ...
        setup() {
          ...
          const click = () => sampleKey.value++;

          return { data, sampleKey, click };
        },
      })
    );
  ...
  it("should revalidate if a function key changes identity", async () => {
    const closureFunctions: Obj<() => Promise<string>> = {};
    const closureFactory = (id: string) =>
      closureFunctions[id] ||
      (closureFunctions[id] = () => Promise.resolve(`data-${id}`));
    const fetcher = (fn: Function) => fn();
    ...
    // const closureSpy = vi.spyOn(closureFunctions, 'first');
  ...
  it("should cleanup state when key turns to empty", async () => {
    const component = mount(
      defineComponent({
        template: `{{ isValidating }} <button @click="click"></button>`,
        setup() {
          const count = ref(1);
          const key = computed(() =>
          ...
          const click = () =>
            (count.value = count.value === 2 ? -1 : count.value + 1);

          return { isValidating, count, click };
        },
      })
    );
...
describe("useSWR - config callbacks", () => {
  it("should trigger the onSuccess event with the latest version of the onSuccess callback", async () => {
    let state: string | null | void = null;
    let count = 0;
    ...
    beforeAll(async () => {
      // await component.find("button").trigger("click");
      await component.findComponent(Block).vm.revalidate();
      await wait(201);
      await component.vm.$nextTick();
